...
router.post('/comment',  util.isAuthenticatedJSON, (req, res) => {
    ...
    let s = req.body.workoutDurationSeconds !== '' ? req.body.workoutDurationSeconds  : '00'
    let m = req.body.workoutDurationMinutes !== '' ? req.body.workoutDurationMinutes  : '00'
    let h = req.body.workoutDurationHours !== '' ? req.body.workoutDurationHours  : '00'
    let workoutDurationInSeconds = parseInt(s) + (parseInt(m) * 60) + (parseInt(h) * 60 * 60 )
    let body = `${POST_TITLE} - @${author} - ${req.body.workoutType} - ${req.body.distance}${req.body.distanceUnit} - ${h}:${m}:${s}`
    let customJson = {
      app: 'move.app/v0.1.0',
