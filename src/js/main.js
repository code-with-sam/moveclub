...
// Formts the response from GetState into a more usuable object
...
function processWorkouts(workouts) {
    ...
    $wrk.prepend(`<div class="workout--date-divider"><p class="workout__date">Today</p></div>`)
...
console.log($('.vote-weight__value').data('vote-weight'))
  e.preventDefault()
  $.post({
    url: '/post/vote',
    dataType: 'json',
    ...
    if (response.error) {
      ...
      $(`<span class="notification">${response.error}</span>`).insertAfter($voteButton.parent())
